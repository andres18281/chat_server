...
socket.on("connection", function (client){
    client.on("join", function(name){
          if(name.id != null){ // si existe un id enviado por el cliente, no genera id
            ...
            people[client.id] = {"id":client.id,"nombre":name.name,"cargo":name.tipo};
          }else{ // en caso de que no exisa el id, socket.io genera el id
            var unique_id = client.id;
            client.id = unique_id.replace("/#","");
            people[client.id] = {"id":client.id,"nombre":name.name,"cargo":name.tipo};
          }
          client.emit("id_user",{"id":people[client.id].id,"nombre":name.name,"cargo":name.tipo});
    ...
    client.on("disconnect", function(){
      //setTimeout(function () {
        ...
        //if (!people[client.id]) {
          ...
          console.log("ha dejado "+people[client.id].name);
